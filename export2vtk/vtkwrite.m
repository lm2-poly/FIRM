%Exports data to vtk (easier visualization)
%This function is initially developed by https://www.mathworks.com/matlabcentral/fileexchange/47814-vtkwrite-exports-various-2d-3d-data-to-paraview-in-vtk-file-format
%but is very slightly modified to allow the extraction of surface data (see
%POLYDATA section)
...
%                 fwrite(fid, reshape(varargin{sidx(ii)+2},1,n_elements),'float','b');
    ...
    fprintf(fid, ['\nPOINT_DATA ' num2str(new_n_elements)]);
    % Parse remaining argument.
    vidx = find(strcmpi(varargin,'VECTORS'));
    sidx = find(strcmpi(varargin,'SCALARS'));
    if vidx~=0
        if new_n_elements == n_elements + 1
            vidx(end+1) = 0;
        elseif new_n_elements == n_elements + 2
            vidx(end+1) = 0; vidx(end+1) = 0;
        end
        for ii = 1:length(vidx)
            title = varargin{vidx(ii)+1};
            % Data enteries begin with a keyword specifying data type
