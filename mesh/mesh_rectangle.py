#Meshes a rectangle based on its dimension using GMSH
#Author: Danick Lamoureux
#Creation date: 2023-05-20
#See mesh_rectangle.m for more information
...
def mesh_rectangle(L, W, mesh_size, show = False):
    G1 = [0, 0, 0]
    G2 = [L, 0, 0]
    G3 = [L, W, 0]
    G4 = [0, W, 0]
    ...
    for i in range(4):
        gmsh.model.geo.addLine(i, np.mod(i+1,4), 4+i)
    gmsh.model.geo.addCurveLoop(curveTags=[4,5,6,7], tag = 8)
...
if __name__ == '__main__':
    ...
    nodes, simplices = mesh_rectangle(L, W, mesh_size, show)
