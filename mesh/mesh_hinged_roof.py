#Meshes a hinged roof based on the description from 
...
#See mesh_hinged_roof.m for more information
...
def hinged_roof(L, theta, R, mesh_size, show = False):
    G0 = [0,0,-R]
    G1 = [0,0,0]
    G2 = [0,L,0]
    G3 = [R*np.sin(theta),L,R*(np.cos(theta)-1)]
    G4 = [R*np.sin(theta), 0,R*(np.cos(theta)-1)]
    G5 = [0,L,-R]
    ...
    gmsh.model.geo.addPoint(G0[0], G0[1], G0[2], mesh_size, 0)
    gmsh.model.geo.addPoint(G1[0], G1[1], G1[2], mesh_size, 1)
    gmsh.model.geo.addPoint(G2[0], G2[1], G2[2], mesh_size, 2)
    gmsh.model.geo.addPoint(G3[0], G3[1], G3[2], mesh_size, 3)
    gmsh.model.geo.addPoint(G4[0], G4[1], G4[2], mesh_size, 4)

    gmsh.model.geo.addPoint(G5[0], G5[1], G5[2], mesh_size, 5)

    gmsh.model.geo.addLine(1, 2, 0)
    gmsh.model.geo.addCircleArc(2, 5, 3, 1)
    gmsh.model.geo.addLine(3, 4, 2)
    gmsh.model.geo.addCircleArc(4, 0, 1, 3)

    gmsh.model.geo.addCurveLoop([0,1,2,3], tag = 4)

    gmsh.model.geo.addSurfaceFilling([4],1)
...
if __name__ == '__main__':
    ...
    print("Show = "+str(show))
    nodes, simplices = hinged_roof(L, theta, R, mesh_size, show = True)
