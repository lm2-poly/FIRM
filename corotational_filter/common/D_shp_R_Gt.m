...
function [K_shp_R_Gt]=D_shp_R_Gt(v, shp_R, G, P, flt_I, n)

% This function computes the tensor:
% d_(shp_R*G'*v)_d_a, with constant v
%
% input:
%
...
% output:
%
% K_shp_R_Gt(6nx6n)         tensor d_(shp_R*G'*v)_d_a, with constant v
%
...
% compute Spin_stack(G'*v, n)*G
Spin_stack_Gt_v_G=Spin_stack(G'*v, n)*G;

% compute d_(shp_R*G'*v)_d_a, with constant v
% equation (111) or (118)
K_shp_R_Gt = -shp_R*( ...
    ... % contribution from derivative of shp_R
    + Spin_stack_Gt_v_G ...
    ... % contribution from derivative of G'
    + Spin_stack_Gt_v_G'*flt_I*P ...
    )*shp_R';
