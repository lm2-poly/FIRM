%Reads the data to plot the follower deformation of a plate
...
for i = 1:length(list)
    ...
    if length(item(:)) > 8
        if item(1:8) == "solution"
            fprintf(item+"\n");
            startind = find('P' == item);
            Fs(end+1) = str2num(item(startind+2:end-4));
            res = open(item);
...
f1 = figure();
hold on;
plot(w/Lx, F*Lx^3/(E*I), 'k');
scatter(modelw(:,1), modelw(:,2), 'ok');
plot(u/Lx,F*Lx^3/(E*I), '--k');
